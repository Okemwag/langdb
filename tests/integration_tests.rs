...
fn test_join_operations() {
    ...
    // Create tables
    assert!(execute_sql_no_result(&executor, "CREATE TABLE users (id INTEGER, name TEXT)").is_ok());
    assert!(execute_sql_no_result(&executor, "CREATE TABLE orders (id INTEGER, user_id INTEGER, amount INTEGER)").is_ok());

    // Insert data
    assert!(execute_sql_no_result(&executor, "INSERT INTO users VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')").is_ok());
    assert!(execute_sql_no_result(&executor, "INSERT INTO orders VALUES (101, 1, 100), (102, 1, 200), (103, 2, 300), (104, 4, 400)").is_ok());

    // Test INNER JOIN
    // Note: The parser expects "JOIN table ON left = right"
    let join_sql = "SELECT users.name, orders.amount FROM users JOIN orders ON users.id = orders.user_id";
    let result = execute_sql(&executor, join_sql).unwrap();
    
    // Should have 3 rows: Alice (100), Alice (200), Bob (300). Charlie has no orders. Order 104 has no user.
    assert_eq!(result.rows.len(), 3);

    // Verify content (schema projection should work)
    // Row order depends on implementation (nested loop: likely users outer, orders inner)
    // Alice (id 1) matches 101 and 102.
    // Bob (id 2) matches 103.
    
    // We can't easily guarantee order without ORDER BY, but nested loop usually preserves insertion order of outer table
    
    // Let's just check existence of values
    let names: Vec<String> = result.rows.iter().map(|r| r.values[0].to_string()).collect();
    let amounts: Vec<String> = result.rows.iter().map(|r| r.values[1].to_string()).collect();
    ...
    let join_where_sql = "SELECT users.name, orders.amount FROM users JOIN orders ON users.id = orders.user_id WHERE orders.amount > 150";
    let result = execute_sql(&executor, join_where_sql).unwrap();
    assert_eq!(result.rows.len(), 2); // Alice (200), Bob (300)
}
